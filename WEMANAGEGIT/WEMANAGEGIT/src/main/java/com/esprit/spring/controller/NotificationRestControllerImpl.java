...
public class NotificationRestControllerImpl {
    ...
    @PostMapping("/add-notification/{user_id}")
	@ResponseBody
	public Notification ajouterNotif2(@RequestBody Notification notification,@PathVariable("user_id") Long user_id) {
		Notification not = notificationService.ajouterNotif2(notification,user_id);
		return not ;
	
	}
    @GetMapping("/getNotif/{id}")
   	@ResponseBody
   	public Notification listnotifById(@PathVariable("id") Long id) {
   		Notification not = notificationService.listnotifById(id);
   		return not ;
