...
public class FeedBackRestControllerImpl {
	...
	public void deleteFeedById(@PathVariable("id") Long ide) {
	   ...
	   public void editDep(@PathVariable Long id, @RequestBody FeedBack feed) {
	    ...
	    @PostMapping("/add-Feed/{jdf_id}")
			@ResponseBody
			public FeedBack ajouterFeedBack2(@RequestBody FeedBack feedback,@PathVariable("jdf_id") Long jdf_id) {
	    	FeedBack feedBack = feedService.ajouterFeedBack2(feedback, jdf_id);
				return feedback ;
			
			}
		    @GetMapping("/getFeed/{id}")
		   	@ResponseBody
		   	public FeedBack listfeedById(@PathVariable("id") Long id) {
		    FeedBack fee = feedService.listfeedById(id);
		   		return fee ;
