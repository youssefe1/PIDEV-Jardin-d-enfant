	...
	public void deleteMessageById(@PathVariable("id") Long ide) {
	   ...
	   public void editDep(@PathVariable Long id, @RequestBody Message message) {
		...
		public void mettreAjourBodyByMessageId(@PathVariable("body") String body, @PathVariable("id") Long id) {
		    ...
		    @GetMapping("/getMessagetrier")
			@ResponseBody
			public List<String> getAllMessageTrieJPQL() {

				return messageService.getAllMessageTrieJPQL();
			}

		    @PostMapping("/add-mes/{user_id}")
			@ResponseBody
			public Message ajouterMessage2(@RequestBody Message message,@PathVariable("user_id") Long user_id) {
		    	Message mes = messageService.ajouterMessage2(message, user_id);
				return mes ;
			
			}
		    @GetMapping("/getMes/{id}")
		   	@ResponseBody
		   	public Message listmessageById(@PathVariable("id") Long id) {
		    	Message mes = messageService.listmessageById(id);
		   		return mes ;
